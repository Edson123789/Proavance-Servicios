...
{
    [Route("api/FurRegistrarPagosV2")]
    public class FurRegistrarPagosV2Controller : Controller
    {
        private readonly integraDBContext _integraDBContext;
        public FurRegistrarPagosV2Controller(integraDBContext integraDBContext)
        {
        ...
        public ActionResult ObtenerCombosFurPago()
        {
            ...
            try
            {
                ...
                comboFurPago.ListaSunatDocumento = repSunatDocumento.ObtenerElementosSunatDocumentoConId();
        ...
        [Route("[Action]")]
        [HttpPost]
        public ActionResult ObtenerRucNombreProveedorComprobanteAutocomplete([FromBody] Dictionary<string, string> Filtros)
        {
            try
            {
                ProveedorRepositorio repProveedorRep = new ProveedorRepositorio();
                ...
                return Ok(repProveedorRep.ObtenerProveedorPorRuc(""));
            }
            catch (Exception Ex)
            {
                return BadRequest(Ex.Message);
        ...
        [Route("[action]")]
        [HttpGet]
        public ActionResult ObtenerFursAsociadosAComprobante(int idComprobante)
        {
            try
            {

                ComprobantePagoRepositorio repComprobantePagoRep = new ComprobantePagoRepositorio();
                var listaObtenerComprobantes = repComprobantePagoRep.ObtenerFursAsociados(idComprobante);
                return Ok(listaObtenerComprobantes);
        ...
        public ActionResult PagoMasivo([FromBody] CompuestoFurRegistrarPagosDTO Json)
        {
            try
            {
                ...
                using (TransactionScope scope = new TransactionScope())
                {                                        
                    foreach (var idFur in Json.FurSeleccionados)
                    {
                        var correlativo = repFurPagoRep.ObtenerFurPago(idFur);
                        furBO = repFur.FirstById(idFur);

                        FurPagoBO furPago = new FurPagoBO();
                        furPago.IdFur = idFur;
                        furPago.IdComprobantePagoPorFur = repComPagoPorFur.ObtenerIdComprobantePorFur(Json.IdComprobante, idFur);
                        furPago.NumeroPago = correlativo;
                        furPago.IdMoneda = Json.FurPago.IdMoneda;
                        ...
                        furPago.FechaCobroBanco = Json.FurPago.FechaCobroBanco;
                        if (Json.FurPago.IdMoneda == 19)
                        {
                            furPago.PrecioTotalMonedaDolares = repComPagoPorFur.ObtenerMontoPorComprobanteFur(Json.IdComprobante, idFur);
                            furPago.PrecioTotalMonedaOrigen = 0;
                        }
                        else
                        {
                            furPago.PrecioTotalMonedaDolares = 0;
                            furPago.PrecioTotalMonedaOrigen = repComPagoPorFur.ObtenerMontoPorComprobanteFur(Json.IdComprobante, idFur);
                        }
                        furPago.IdComprobantePago = Json.IdComprobante;
                        furPago.UsuarioCreacion = Json.Usuario;
                        furPago.FechaCreacion = DateTime.Now;
                        furPago.UsuarioModificacion = Json.Usuario;
                        furPago.FechaModificacion = DateTime.Now;
        ...
        [Route("[action]/{IdComprobante}")]
        [HttpGet]
        public ActionResult BuscarFurAsociados(int IdComprobante)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                ...
                FurRepositorio repFurRep = new FurRepositorio();
                List<FurAsociadoComprobanteDTO> furAsociados = new List<FurAsociadoComprobanteDTO>();

                //furAsociados = repFurRep.ObtenerFurAsociadosPorComprobante(IdComprobante);

                var valor = repFurRep.ObtenerFurAsociadosPorComprobante(IdComprobante);
                foreach (var valoresMonto in valor)
                {
                    if (valoresMonto.MontoAmortizar > 0)
                    {
                        furAsociados.Add(valoresMonto);
                    }
                }
                return Ok(furAsociados);
        ...
        [Route("[action]")]
        [HttpPost]
        public ActionResult ObtenerFurParaAsociar([FromBody] CompuestoFurPorAsociarDTO compuestofur)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            try
            {
                ...
                FurRepositorio repFurRep = new FurRepositorio();
                List<FurPorAsociarDTO> furporAsociar = new List<FurPorAsociarDTO>();

                furporAsociar = repFurRep.ObtenerFurParaAsociar(compuestofur);
                   
                return Ok(furporAsociar);
        ...
        [Route("[action]")]
        [HttpGet]
        public ActionResult ObtenerComprobantesPorFurParaPago(int idFur)
        {
            if (!ModelState.IsValid)
            {
                return BadRequest(ModelState);
            ...
            {
                ComprobantePagoPorFurRepositorio repComprobantePagoPorFur = new ComprobantePagoPorFurRepositorio();
                var ListaComprobantePagoPorFur = repComprobantePagoPorFur.ObtenerComprobantesPorFurParaPago(idFur);
